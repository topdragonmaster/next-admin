...
import { deleteStockCode, createStockCode, getStockCode } from "@/services/stockCode";
import { deleteStockCode as deleteStockCodeAction } from "@/store/actions/stockCodeAction";
...
const Station_Account = () => {
  ...
  const [stockCodePopupOpen, setStockCodePopupOpen] = useState(false)
  ...
  console.log(stations, accounts, stockCodes)
  ...
  const handleStockCodeDelete = async (_id) => {
    const data = await deleteStockCode(_id)
    if (data.success) {
      dispatch(deleteStockCodeAction(_id))
      toast.success(data.message)
      return
    }
    toast.error(data.message)
  ...
  const handleStockCodeCreate = async (e) => {
    e.preventDefault()
    if (formData.stockCode === '') {
      toast.warning('Input the stockcode name')
      return
    }

    const data = await createStockCode({ name: formData.stockCode })
    console.log(data)
    if (data.success) {
      await getStockCode(dispatch)
      toast.success(data.message)
    } else {
      toast.error(data.message)
    }
    setStockCodePopupOpen(false)
  }
  ...
  return (
    <div className="mx-auto">
      <Breadcrumb pageName="Station, StockCode, Account " />
          ...
          <div className="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
            <div className="p-4 md:p-6 xl:p-7.5">
              <div className="flex items-start justify-between">
                ...
                <button
                  onClick={() => setStockCodePopupOpen(true)}
                  className="flex items-center gap-2 rounded-full bg-primary py-3 px-3 font-medium text-white hover:bg-opacity-80"
                >
                  <svg
                    className="fill-current"
            ...
            <div className="p-4 md:p-6 xl:p-7.5">
              <div className="flex flex-col gap-7">
                {stockCodes.length > 0 ? stockCodes.map(({ name, _id }) => (
                  <div className="flex items-center justify-around gap-3" key={_id}>
                    <div className="w-2/12 xl:w-3/12">
                      <div className="flex items-center gap-4">
                        <span className="font-medium xl:block">
                    ...
                    <div className="hidden w-2/12 2xsm:block md:w-1/12">
                      <button onClick={() => handleStockCodeDelete(_id)} className="mx-auto block hover:text-meta-1">
                        <svg
                          className="mx-auto fill-current"
                          width="22"
                          height="22"
      ...
      <div
        className={`fixed top-0 left-0 z-99999 flex h-screen w-full justify-center overflow-y-scroll bg-black/80 py-5 px-4 ${stockCodePopupOpen === true ? 'block' : 'hidden'
          }`}
      >
        <div className="relative m-auto w-full max-w-180 rounded-sm border border-stroke bg-gray p-4 shadow-default dark:border-strokedark dark:bg-meta-4 sm:p-8 xl:p-10">
          <button
            onClick={() => setStockCodePopupOpen(false)}
            className="absolute right-1 top-1 sm:right-5 sm:top-5"
          >
            <svg
              className="fill-current"
          ...
          <form onSubmit={handleStockCodeCreate}>
            <div className="mb-5">
              <label
                htmlFor="taskTitle"
                className="mb-2.5 block font-medium text-black dark:text-white"
              ...
              <input
                ...
                placeholder="Enter StockCode name"
                className="w-full rounded-sm border border-stroke bg-white py-3 px-4.5 focus:border-primary focus-visible:outline-none dark:border-strokedark dark:bg-boxdark dark:focus:border-primary"
              />
            </div>
