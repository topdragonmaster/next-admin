...
const Trade = () => {
  ...
  const tableData = useMemo(() => trades?.map(trade => ({
    _id: trade._id,
    accountId: trade?.account?._id,
    stockCode: trade.stockCode,
    stationName: trade?.account?.station?.name,
  ...
  const handleTradeDelete = async (_id) => {
    console.log(_id)
    const data = await deleteTrade(_id)
    if (data.success) {
      dispatch(deleteTradeAction(_id))
  ...
  const handleTradeEditOpen = async (row) => {
    setIsCreate(false);
    setFormData({
      _id: row._id,
  ...
  const handleTradeEdit = async (e) => {
    e.preventDefault()
    if (formData.price === 0) {
      toast.warning('Input the Price')
      return
    }
    ...
    if (formData.expiration === 0) {
      toast.warning('Input the expiration days')
      return
    }
    ...
    const data = await updateTrade({ ...formData })
    if (data.success) {
      await getTrade(dispatch)
      toast.success(data.message)
    } else {
      toast.error(data.message)
    }
  ...
  const handleTradeCreate = async (e) => {
    e.preventDefault()
    ...
    const data = await createTrade({ ...formData })
    if (data.success) {
      await getTrade(dispatch)
      toast.success(data.message)
    } else {
  ...
  return (
    <div className="mx-auto max-w-7xl">
                  ...
                  {page.map((row, key) => {
                    ...
                    return (
                      <tr {...row.getRowProps()} key={key}>
                        ...
                        <td className="item-center" key={key}>
                          <DropdownDefault
                            onEdit={() => handleTradeEditOpen(row.original)}
                            onDelete={() => handleTradeDelete(row.original._id)}
                          />
                        </td>
                      </tr>
                    );
      ...
      <div
        className={`fixed top-0 left-0 z-999 flex h-screen w-full justify-center overflow-y-scroll bg-black/80 py-5 px-4 ${tradePopupOpen === true ? 'block' : 'hidden'
          }`}
      >
        <div className="relative m-auto w-full max-w-180 rounded-sm border border-stroke bg-gray p-4 shadow-default dark:border-strokedark dark:bg-meta-4 sm:p-8 xl:p-10">
          <button
            onClick={() => setTradePopupOpen(false)}
          ...
          <form onSubmit={isCreate ? handleTradeCreate : handleTradeEdit}>
            <div className="p-6.5">
              ...
              <div className="mb-4.5 flex flex-col gap-6 xl:flex-row">
                <div className="w-full xl:w-1/2">
                  <label className="mb-2.5 block text-black dark:text-white">
                    Expiration <span className="text-meta-1">*</span>
                  </label>
                  <input
                    type="number"
                    placeholder="Enter the expiration"
